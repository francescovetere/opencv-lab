...
void interpolation_neighbors(const cv::Mat& input, int pixel_r, int pixel_c, int window_size, cv::Mat& output) {
	float sum = 0;
	int num_pixels = 0;
	for(int r = -window_size; r < window_size/2; ++r) {
		for(int c = -window_size; c < window_size/2; ++c) {
	...
	output.at<uint8_t>(pixel_r, pixel_c) = sum/num_pixels;
} 
...
int main(int argc, char **argv)
{
	...
	while(!exit_loop)
	{
		...
		cv::Mat output_B(input_img.rows, input_img.cols, CV_8UC1, cv::Scalar(0));
		cv::Mat output_G(input_img.rows, input_img.cols, CV_8UC1, cv::Scalar(0));
		cv::Mat output_R(input_img.rows, input_img.cols, CV_8UC1, cv::Scalar(0));
		...
		for(int r = 0; r < input_img.rows; ++r) {
			for(int c = 0; c < input_img.cols; ++c) {
				if(output_B.at<uint8_t>(r, c) == 0)
					interpolation_neighbors(input_img, r, c, 3, output_B);
				if(output_G.at<uint8_t>(r, c) == 0) 
					interpolation_neighbors(input_img, r, c, 3, output_G);
				if(output_R.at<uint8_t>(r, c) == 0) 
					interpolation_neighbors(input_img, r, c, 3, output_R);
			}
		}
