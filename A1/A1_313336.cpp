...
void contrast_stretching(const cv::Mat& input, cv::Mat& output) {
	double min_pixel = 0, max_pixel = 0;
	cv::minMaxLoc(input, &min_pixel, &max_pixel);

    // for(int i = 0; i < input.rows; i++)
    // {
    //     const float* Ki = input.ptr<float>(i);
...
void conv(const cv::Mat& image, const cv::Mat& kernel, cv::Mat& out, int stride = 1) {
	...
	std::cout << "input rows: " << image.rows << std::endl;
	std::cout << "output rows: " << out.rows << std::endl;
	std::cout << "padding rows: " << padding_rows << std::endl;
...
void sobel(const cv::Mat& image, cv::Mat& magnitude, cv::Mat& orientation) {
	...
	for(int r = 0; r < image.rows; ++r) {
		for(int c = 0; c < image.cols; ++c) {
				...
				*((float*) &(magnitude.data[(r*magnitude.cols + c)*magnitude.elemSize()]))
				...
				// if(*((float*) &(magnitude.data[(r*magnitude.cols + c)*magnitude.elemSize()])) > 255) {
				// 	std::cout << "deriv x: " << current_derivative_x << " " <<
				// 	"deriv y: " << current_derivative_y << " " <<
				// 	"magnitude val: " << *((float*) &(magnitude.data[(r*magnitude.cols + c)*magnitude.elemSize()])) << std::endl;
				// }

				*((float*) &(orientation.data[(r*orientation.cols + c)*orientation.elemSize()])) 
				= atan2(current_derivative_y, current_derivative_x);

				// std::cout << *((float*) &(orientation.data[(r*orientation.cols + c)*orientation.elemSize()])) << std::endl;                                                       
		}
	}
    ...
    // {
    //     const float* g_x = derivative_x.ptr<float>(i);
    //     const float* g_y = derivative_y.ptr<float>(i);
    //     float* intensity = magnitude.ptr<float>(i);

    //     for(int j = 0; j < magnitude.cols; j++){
    //         intensity[j] = sqrt((g_x[j]*g_x[j])+(g_y[j]*g_y[j]));
    //     }
    // }
	...
	// DEBUGGING
	cv::Mat derivative_x_tmp;
	contrast_stretching(derivative_x, derivative_x_tmp);

	cv::Mat derivative_y_tmp;
	contrast_stretching(derivative_y, derivative_y_tmp);

	cv::namedWindow("derivative_x_tmp", cv::WINDOW_AUTOSIZE);
	cv::imshow("derivative_x_tmp", derivative_x_tmp);

	cv::namedWindow("derivative_y_tmp", cv::WINDOW_AUTOSIZE);
	cv::imshow("derivative_y_tmp", derivative_y_tmp);
