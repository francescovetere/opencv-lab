/**
 * Caricate l'immagine di Lenna e fate un downsample 2x delle sole colonne:
 * una nuova immagine di dimensioni w/2 e h ottenuta prendendo una colonna si ed una no
 */
//OpenCV
		...
		{	
			for(int u = 0;u < output_img.cols; ++u)
			{
				...
				{	//per ogni pixel dell'immagine di output, copio il pixel corrispondente nell'immagine di input, con righe e colonne moltiplicate per 2
					...
					= input_img.data[(v*input_img.cols + u*DOWNSAMPLE_FACT)*input_img.channels() + k];
				}
			}
		}
