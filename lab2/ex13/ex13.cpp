/**
 * Demosaicatura SIMPLE :
 * - l'immagine di uscita sara' di dimensioni w x h, a colori (CV_8UC3)
 * - ogni 4 pixel GBRG dell'immagine originale estraete 3 canali di colore nel seguente modo:
 *   R = canale rosso del pattern
 *   G = media dei canali G del pattern
 *   B = canale blu del pattern
...
int main(int argc, char **argv)
{
	...
	while(!exit_loop)
	{
		...
		// Output input_img
		cv::Mat output_img(input_img.rows,input_img.cols,CV_8UC3,cv::Scalar(1,2,3));
		...
		for(int v = 0; v < output_img.rows; ++v) {
			...
			for(int u = 0; u < output_img.cols; ++u) {
				...
				output_img.data[(u + v*output_img.cols)*3] = result_b; //B
				output_img.data[(u + v*output_img.cols)*3 + 1] = result_g; //G
				output_img.data[(u + v*output_img.cols)*3 + 2] = result_r; //R
